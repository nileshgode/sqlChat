...
# --- Module-level variables for tools and LLM ---
llm = None
tools = []
...
def generate_query(state: MessagesState):
    ...
    generate_query_system_prompt = f"You are an agent designed to interact with a SQL database. Given an input question, create a syntactically correct query to run."
...
def check_query(state: MessagesState):
    ...
    check_query_system_prompt = "You are a SQL expert. Double check the given query for common mistakes. If there are mistakes, rewrite it. If not, reproduce the original query."
...
def create_sql_agent_graph(llm_instance, db):
    """
    Initializes tools and compiles the LangGraph SQL agent.
    """
    global llm, tools, get_schema_tool, run_query_tool, list_tables_tool
    llm = llm_instance

    # Initialize tools and assign them to the module-level variables
